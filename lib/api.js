...
module.exports = Class.create({
	...
	requireValidEventData: function (event, callback) {
		...
		// 2026-09-06: disabling this hardening (inherited from upstream). Notes allowed to have basic html tags (b/i/u) to display on tooltip on schedule page (as a feature)
		// tooltips protected from XSS already
	...
	requireValidEventParams: function(params, plugin, callback) {
		...
		if (plugin.params) plugin.params.forEach( function(param) {
			...
			if(param.id === 'script') {
				allowed_params[ 'lang'] = true;
				allowed_params[ 'theme'] = true;
			}
		} );
