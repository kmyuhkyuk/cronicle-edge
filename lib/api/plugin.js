...
module.exports = Class.create({
	...
	api_create_plugin: function (args, callback) {
		...
		this.loadSession(args, async function (err, session, user) {
			...
			let alreadyExist = await self.validateUnique('global/plugins', plugin, ["id", "title"])
			if (alreadyExist > 0) {
				return self.doError('event', `Plugin with title or id ${plugin.title}/${plugin.id} already exists`, callback);
			}
	...
	api_update_plugin: function (args, callback) {
		...
		this.loadSession(args, async function (err, session, user) {
			...
			self.storage.listFind('global/plugins', { id: params.id }, async function (err, plugin) {
				if (err || !plugin) {
				...
				if (plugin.title != params.title) {
					let titleExists = await self.validateUnique('global/plugins', params, ["title"])
					if (titleExists > 0) {
						return self.doError('event', `Plugin with title "${params.title}" already exists`, callback);
					}
				}
	...
	requireValidPluginParams: function(params, callback) {
		...
		for (var idx = 0, len = params.length; idx < len; idx++) {
			...
			if (!param.title) return this.doError('plugin', `Plugin Parameter [${param.id}] Title is missing.`, callback);
			if (typeof(param.title) != 'string') return this.doError('plugin', `Plugin Parameter [${param.id}] Title is not a string.`, callback);
			if (param.title.match(/[<>]/)) return this.doError('plugin', `Plugin Parameter [${param.id}] Title contains HTML metacharacters.`, callback);
			...
			switch (param.type) {
				case 'text':
					...
					if (!('value' in param) || (typeof(param.value) != 'string')) return this.doError('plugin', `Plugin Parameter [${param.id}] value is invalid.`, callback);
					if (param.value.match(/[<>]/)) return this.doError('plugin', `Plugin Parameter [${param.id}] text value contains HTML metacharacters.`, callback);
				break;
