...
module.exports = Class.create({
	...
	// global lists a data folder migrated from classic Cronicle does not have
	requiredLists: ['global/secrets', 'global/conf_keys'],
	...
	isManagerGroup: function (group) {
		// classic Cronicle called the manager node "master"; a data folder migrated from
		// classic has server_groups records with `master` set and no `manager` field, so
		// fall back to `master` on the read path (stored records are never rewritten here).
		// `manager` decides alone once it is present, so an administrator can revoke
		// eligibility on a group that still carries the classic flag.
		if (typeof group.manager !== 'undefined') return !!group.manager;
		return !!group.master;
	},
		...
		this.storage.listGet('global/servers', 0, 0, function (err, servers) {
			...
			self.storage.listGet('global/server_groups', 0, 0, function (err, groups) {
				...
				for (var idx = 0, len = groups.length; idx < len; idx++) {
					...
					if (self.isManagerGroup(group) && regexp && self.server.hostname.match(regexp)) {
						eligible = true;
	...
	createRequiredLists: function (callback) {
		// a data folder migrated from classic Cronicle has neither of the two global lists this
		// version expects, and global/secrets is read on every job launch (jobs fail without it).
		// a key that is already there is never touched, so this is a no-op on a healthy install.
		const self = this;

		// gomanager() can run more than once in a process (a failover chain that stalls past
		// manager_ping_timeout fires another one), and two overlapping passes would both see a
		// key as missing and create it twice, so only the first pass of a process does the work
		if (this.requiredListsChecked) { if (callback) callback(); return; }
		this.requiredListsChecked = true;

		var setup = Tools.copyHash(require('../sample_conf/setup.json'), true);

		async.eachSeries(this.requiredLists,
			function (key, callback) {
				self.storage.get(key, function (err) {
					if (!err) return callback();
					if (err.code != 'NoSuchKey') {
						// on any other error we cannot tell what is in there, so the key is left alone
						self.logError('storage', "Failed to check required list: " + key + ": " + err);
						return callback();
					}

					// the sample entries flagged optional are examples, not something edge needs
					var seeds = setup.storage.filter(function (params) {
						return (params[0] == 'listPush') && (params[1] == key) && !params[2].optional;
					}).map(function (params) { return params[2]; });

					self.storage.listCreate(key, {}, function (err) {
						if (err) {
							self.logError('storage', "Failed to create required list: " + key + ": " + err);
							return callback();
						}
						self.logDebug(3, "Created required list missing from this data folder: " + key);

						async.eachSeries(seeds,
							function (item, callback) {
								// massage a few params (same as the setup command)
								if (item.created) item.created = Tools.timeNow(true);
								if (item.modified) item.modified = Tools.timeNow(true);

								self.storage.listPush(key, item, function (err) {
									// an empty required list is a valid end state -- the key exists, so the
									// next manager start leaves it alone and the daemon keeps running
									if (err) self.logError('storage', "Failed to seed required list: " + key + ": " + err);
									callback();
								});
							},
							callback
	...
	importLegacyPlugins: function (callback) {
		// a data folder migrated from classic Cronicle carries only the three plugins classic
		// itself shipped, so this version's own plugins are imported once. the `manager` flag
		// stamped onto the server groups below is the fingerprint that says it already happened
		const self = this;

		if (this.legacyImportChecked) { if (callback) callback(); return; }
		this.legacyImportChecked = true;

		// the import has to see what is on disk, and two things stand between it and that:
		// the manager's RAM cache for global/*, which put() primes before the engine write
		// completes and never reverts, and listGet, which trims a page to the count in the
		// list header, so an item whose page write landed while its header write failed is
		// invisible. read the header and every page straight from the engine instead, and
		// say whether the header still agrees with the items actually there
		var readPluginsOnDisk = function (callback) {
			self.storage.engine.get('global/plugins', function (err, list) {
				if (err) return callback(err);
				var items = [];
				var page_idx = list.first_page;
				var next = function () {
					if (page_idx > list.last_page) return callback(null, items, items.length == list.length);
					self.storage.engine.get('global/plugins/' + page_idx, function (err, page) {
						if (err) return callback(err);
		...
		this.storage.listGet('global/server_groups', 0, 0, function (err, groups) {
			if (err) {
				self.logError('storage', "Failed to fetch server groups: " + err);
				if (callback) callback();
				return;
			}

			// classic wrote `master` and never wrote `manager`, so a record with one and not
			// the other has not been through this import
			var legacy_groups = groups.filter(function (group) {
				return (group.manager === undefined) && (group.master !== undefined);
			});
			if (!legacy_groups.length) { if (callback) callback(); return; }

			readPluginsOnDisk(function (err, plugins, consistent) {
				if (err) {
					self.logError('storage', "Failed to fetch plugins: " + err);
					if (callback) callback();
					return;
				}
				if (!consistent) {
					// a header that disagrees with its pages is a list the storage layer cannot
					// repair on its own, and adding to it would only bury the problem deeper
					self.logError('storage', "The plugin list header does not match its pages, leaving the data folder untouched until it is repaired");
					if (callback) callback();
					return;
				}
				...
				plugins.forEach(function (plugin) {
					taken_ids[plugin.id] = true;
					taken_titles[plugin.title] = true;
				});

				var setup = Tools.copyHash(require('../sample_conf/setup.json'), true);

				var imports = setup.storage.filter(function (params) {
					return (params[0] == 'listPush') && (params[1] == 'global/plugins');
				}).map(function (params) {
					return params[2];
				}).filter(function (plugin) {
					// SSHX supersedes the SSH plugin, which is on its way out, so a migrated
					// folder never receives it
					if (plugin.id == 'sshplug') return false;
					// same uniqueness rule the create API enforces, so an import can never
					// produce a record the UI would refuse; the shell, test and http plugins
					// classic ships under the same ids stay as they are
					return !taken_ids[plugin.id] && !taken_titles[plugin.title];
				});

				async.eachSeries(imports,
					function (plugin, callback) {
						plugin.created = Tools.timeNow(true);
						plugin.modified = Tools.timeNow(true);
						self.storage.listPush('global/plugins', plugin, callback);
					},
					function (err) {
						if (err) {
							// leaving the groups unstamped is what lets the next manager start
							// finish the import -- the pushes that did land are skipped by id
							self.logError('storage', "Failed to import legacy plugins: " + err);
							if (callback) callback();
							return;
						}

						// a push that reports success has not necessarily written every page: the
						// list writer keeps only the last completion's error, so a page that failed
						// before the header landed comes back clean. the fingerprint is permanent,
						// so it follows what the list itself shows on disk, never the callbacks alone
						readPluginsOnDisk(function (err, plugins, consistent) {
							if (err) {
								self.logError('storage', "Failed to read back global/plugins after the import: " + err);
								if (callback) callback();
								return;
							}
							if (!consistent) {
								self.logError('storage', "The plugin list header does not match its pages after the import, leaving the server groups unstamped");
								if (callback) callback();
								return;
							}
							var present = {};
							(plugins || []).forEach(function (plugin) { present[plugin.id] = true; });
							var missing = imports.filter(function (plugin) { return !present[plugin.id]; });
							if (missing.length) {
								self.logError('storage', "Legacy plugin import is incomplete, leaving the server groups unstamped: " +
									missing.map(function (plugin) { return plugin.id; }).join(', '));
								if (callback) callback();
								return;
							}

							async.eachSeries(legacy_groups,
								function (group, callback) {
									self.storage.listFindUpdate('global/server_groups', { id: group.id },
										{ manager: parseInt(group.master) ? 1 : 0 }, callback);
								},
								function (err) {
									if (err) {
										self.logError('storage', "Failed to stamp server groups: " + err);
										if (callback) callback();
										return;
									}

									self.logDebug(1, "Data folder migrated from classic Cronicle: imported " +
										imports.length + " plugin(s), stamped " + legacy_groups.length + " server group(s)");

									// browsers that are already logged in hold their own copy of the list
									if (imports.length) self.updateClientData('plugins');
	...
	gomanager: function () {
		...
		// cluster nodes share storage, so only the manager may repair a data folder
		// migrated from classic Cronicle
		this.createRequiredLists(function () {
			self.importLegacyPlugins();
		});
