...
module.exports = Class.create({
	...
	__parent: Component,
	__mixins: [
		require('./oidc') // OpenID Connect Layer Mixin
	],
	...
	startup: function (callback) {
		...
		this.server.on('day', function() { self.oauth_state = {}; });
		// startup complete
		callback();
	},
		...
		return new Promise((resolve, reject) => {
			...
			request.request(String(url), options, function (err, resp, data) {
				...
				try { 
					...
					self.logError('oauth', 'OAuth HTTP response was not valid JSON')
	...
	getOAuthErrorMessage: function(err) {
		...
		return new Promise((resolve, reject) => {
			...
			request.post(String(url), options, function (err, resp, data) {
				...
				try { 
					...
					self.logError('oauth', 'OAuth token response was not valid JSON')
					reject(new Error('response is not json'))
				}
			});
		});
	...
	isMissingStorageError: function(err) {
		return !!err && (err.code === 'NoSuchKey' || err.code === 'ENOENT');
	},

	getSessionId: function(args) {
		return (args._oidc_secrets && args._oidc_secrets.session_id) ||
			args.cookies['session_id'] || args.request.headers['x-session-id'] ||
			args.params.session_id || args.query.session_id;
	},

	deleteSessionRecord: function(session, callback) {
		// Delete OIDC indexes together with their primary session record.
		const self = this;
		if (!session || !session.id) return callback();
		if (session.auth_provider !== 'oidc') {
			return self.storage.delete('sessions/' + session.id, function(err) {
				if (self.isMissingStorageError(err)) err = null;
				callback(err || null);
			});
		}

		const lockKey = 'oidc-session-lock/' + session.id;
		self.storage.lock(lockKey, true, function() {
			...
			self.storage.get('sessions/' + session.id, function(getErr, current) {
				if (getErr && !self.isMissingStorageError(getErr)) return finish(getErr);
				current = current || session;
				self.updateOidcSessionIndexes(current, false, function(indexErr) {
					if (indexErr) return finish(indexErr);
					if (getErr) return finish();
					self.storage.delete('sessions/' + session.id, function(deleteErr) {
						if (!deleteErr || self.isMissingStorageError(deleteErr)) return finish();
						// Keep the index consistent if the primary record could not be deleted.
						self.updateOidcSessionIndexes(current, true, function(restoreErr) {
							finish(restoreErr || deleteErr);
						});
					});
				});
			});
	...
	getOauthConfig: function () {
		...
		const self = this
		let oauth = Tools.copyHash(self.server.config.get('oauth') || {}, true)
		...
		if (oauth.profile) { // merge values from pre-configured profiles (hardoced above)
			...
			oauth = Tools.mergeHashes(Tools.copyHash(profiles[oauth.profile] || {}, true), oauth)
		}

		if (String(oauth.base_url).startsWith('http')) { // inject base_url to relative paths
			let base_url = String(oauth.base_url).replace(/\/+$/, '');
			let urls = ["authorize_url", "token_url", "redirect_uri", "user_url", "issuer", "jwks_url"];
			urls.forEach((url) => {
				if (String(oauth[url]).startsWith('/')) oauth[url] = base_url + oauth[url]
			})
		}

		const required = ["client_id", "client_secret", "authorize_url", "token_url", "redirect_uri", "user_url"];
		for (const prop of required) {
			if (!oauth[prop] || typeof oauth[prop] !== 'string') {
				self.logError('oauth', 'Invalid or missing OAuth configuration property: ' + prop)
				return null
			}
		}
	...
	api_oauth: function (args, callback) {
		...
		let session_id = self.getSessionId(args);
		if(session_id) {
			self.logDebug(3, 'Session is already set, redirecting user back')
		...
		if (oauth) {
			...
			const stateData = {};
			if (String(oauth.scope || '').split(/\s+/).includes('openid')) stateData.nonce = Tools.generateUniqueID(32);
			self.oauth_state[state] = stateData
	...
	api_callback: async function (args, callback) {
		...
		if (args._oidc_secrets) {
			if (args._oidc_secrets.code) params.code = args._oidc_secrets.code;
			if (args._oidc_secrets.state) params.state = args._oidc_secrets.state;
		}
		...
		if (!stateStore[state]) { // state should be set by /login/auth endpoint (see above)
			self.logDebug(3, 'Invalid OAuth state')
		...
		try {
			...
			if(!tokenResponse.access_token) {
				self.logError('oauth', 'OAuth token endpoint did not return an access token')
				return self.doError('login', 'Failed to fetch access token', callback);
			}

			const oidcConfigured = !!(oauth.issuer && oauth.jwks_url);
			const oidcRequired = !!((oauth.backchannel_logout || {}).enabled || (oauth.logout || {}).enabled);
			const rpLogoutEnabled = !!((oauth.logout || {}).enabled);
			if (oidcRequired && !oidcConfigured) {
				throw new Error('oauth.issuer and oauth.jwks_url are required for configured OIDC logout');
			}
			if (rpLogoutEnabled && !tokenResponse.id_token) {
				throw new Error('OIDC token endpoint did not return an ID Token required for RP-initiated logout');
			}
			if (tokenResponse.id_token && oidcConfigured) {
				const verified = await self.verifyOidcIdToken(tokenResponse.id_token, oauth, stateData.nonce);
				oidcClaims = verified.payload;
			}
			...
			// ====== get User Info 
			let userResponse = await self.getJsonAsync(oauth.user_url, {
				method: 'GET',
			...
			if (oidcClaims && userResponse.sub && oidcClaims.sub !== userResponse.sub) {
				throw new Error('OIDC UserInfo subject does not match the ID Token subject');
			}
			if (!oidcClaims && oauth.issuer && oauth.jwks_url && userResponse.sub) {
				const provider = self.validateOidcProviderConfig(oauth);
				oidcClaims = { iss: provider.issuer, sub: userResponse.sub };
			}
			if (oauth.issuer && oauth.jwks_url) {
				oidcMetadata = self.buildOidcSessionMetadata(oauth, oidcClaims, userResponse, tokenResponse.id_token,
					self.server.config.get('secret_key'));
			}
			...
			self.logError(3, 'oauth failed', self.getOAuthErrorMessage(e))
		...
		self.fireHook('before_login', args, function (err) {
			...
			if (oidcMetadata) Object.assign(session, oidcMetadata);

			self.logDebug(6, "Logging user in via OAuth: " + params.username);

			const storeSession = session.auth_provider === 'oidc' ?
				self.storeOidcSession.bind(self) :
				function(localSession, done) {
					self.storage.put('sessions/' + localSession.id, localSession, function(err) {
						if (!err) self.storage.expire('sessions/' + localSession.id, localSession.expires);
						done(err);
					});
				};

			// store session object and its OIDC indexes, if applicable
			storeSession(session, function (err) {
				if (err) {
	...
	api_logout: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_logout', args, function (err) {
				...
				let logoutWarning = null;
				try {
					if (session.auth_provider === 'oidc') {
						const oauth = self.getOauthConfig();
						if (!oauth) throw new Error('OAuth configuration is unavailable');
						logoutLocation = self.buildOidcLogoutLocation(oauth, session, self.server.config.get('secret_key'));
					}
				}
				catch (configErr) {
					logoutWarning = 'Local logout succeeded, but OIDC logout configuration is invalid.';
					self.logError('oauth', 'Invalid OIDC logout configuration: ' + configErr.message);
				}

				// Delete the local session before returning any external logout URL.
				self.deleteSessionRecord(session, function (deleteErr) {
					if (deleteErr) {
						return self.doError('logout', 'Failed to delete local session: ' + deleteErr.message, callback);
					}
					...
					if (logoutLocation) {
						try {
							const ticket = self.createOidcLogoutTicket(logoutLocation, self.server.config.get('secret_key'), 60);
							response.logout_location = self.getBaseLocation('/api/user/oidc_logout_redirect?ticket=' +
								encodeURIComponent(ticket));
						}
						catch (ticketErr) {
							logoutWarning = 'Local logout succeeded, but the OIDC logout redirect could not be created.';
							self.logError('oauth', 'Failed to create OIDC logout redirect ticket: ' + ticketErr.message);
						}
					}
					if (logoutWarning) response.logout_warning = logoutWarning;
					callback(response);
	...
	api_resume_session: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_resume_session', args, function (err) {
				...
				// OIDC resumes share the callback/logout locks so stale state cannot recreate a revoked session.
				const storeSession = session.auth_provider === 'oidc' ?
					self.storeOidcSession.bind(self) :
					function(localSession, done) { self.storage.put('sessions/' + localSession.id, localSession, done); };
				storeSession(session, function (err) {
					if (err) {
	...
	api_delete: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_delete', args, function (err) {
				...
				self.logDebug(6, "Deleting current user session");
				self.deleteSessionRecord(session, function (sessionErr) {
					if (sessionErr) {
						return self.doError('user', 'Failed to delete session: ' + sessionErr.message, callback);
					}
	...
	getClientInfo: function (args, params) {
		...
		if (!params) params = {};
		params.ip = args.ip;
		params.headers = Tools.copyHash(args.request.headers || {});
		['authorization', 'cookie', 'cf-access-jwt-assertion', 'x-api-key', 'x-session-id', 'x-gitlab-token'].forEach(function(name) {
			if (params.headers[name]) params.headers[name] = '[REDACTED]';
		});
		return params;
	},
	...
	loadSession: function (args, callback) {
		...
		var self = this;
		var session_id = self.getSessionId(args);
